...
Vyhledejte `smsprov.log` tyto řádky:
...
- Configuration Manager verze 1902:
 ...
 `7DC6B6F1-E7F6-43C1-96E0-E1D16BC25C14` je identifikátor GUID skriptu pro CMPivot. Tento identifikátor GUID můžete zobrazit také ve [zprávách o stavu auditů CMPivot](cmpivot-changes.md#cmpivot-audit-status-messages).

V dalším kroku vyhledejte ID v okně CMPivot. Toto ID je `ClientOperationID` .

![Okno CMPivot se zvýrazněným ClientOperationID verze 1902](media/cmpivot-client-operationid-1902.png)
...
V nástroji `BgbServer.log` vyhledejte `TaskID` shromážděné údaje z SQL a Všimněte si `PushID` . `TaskID`Je označen `TaskGUID` . Například:
...
Po zjištění informací ze serveru lokality ověřte protokoly klienta. Ve výchozím nastavení se protokoly klienta nacházejí v nástroji `C:\Windows\CCM\Logs` .

V nástroji `CcmNotificationAgent.log` vyhledejte položky protokolu, které vypadají jako následující řádky:  
...
Pokud je v bodu správy povoleno [podrobné protokolování](../../plan-design/hierarchy/about-log-files.md#bkmk_reg-client) , můžete zjistit, jak jsou zpracovávány příchozí zprávy klienta. V nástroji   `MP_RelayMsgMgr.log` vyhledejte `TaskID` .

V tomto `MP_RelayMsgMgr.log` příkladu vidíte ID klienta `(GUID:83F67728-2E6D-4E4F-8075-ED035C31B783)` a `Task ID {9A4E59D2-2F5B-4067-A9FA-B99602A3A4A0}` . ID zprávy se před odesláním do modulu zpracování zprávy přiřadí k odpovědi klienta:
...
Pokud je v nástroji povoleno [podrobné protokolování](../../plan-design/hierarchy/about-log-files.md#bkmk_logoptions) `SMS_MESSAGE_PROCESSING_ENGINE.log` , jsou zpracovány výsledky klienta. Použijte ID zprávy, které jste našli z `MP_RelayMsgMgr.log` . Položky protokolu zpracování jsou podobné jako v následujícím příkladu:
...
V nástroji `BgbServer.log` vyhledejte a `PushID` Zobrazte počet klientů, kteří se nahlásili nebo nezdařili.
...
Vyhledejte `smsprov.log` Tento řádek:
...
V nástroji `BgbServer.log` vyhledejte `TaskID` shromážděné údaje z SQL. Je označený `TaskGUID` . Například:
...
V nástroji `CcmNotificationAgent.log` vyhledejte protokoly, které jsou podobné následujícímu záznamu:  
...
Podívejte `Scripts.log` se na `TaskID` . V následujícím příkladu vidíte `Task ID {F8C7C37F-B42B-4C0A-B050-2BB44DF1098A}` :
...
Pokud zpráva nebyla zpracována, podívejte se do složky Doručená pošta zpráv o stavu. Výchozí umístění doručené pošty je `C:\Program Files\Microsoft Configuration Manager\inboxes\auth\statesys.box\` . Vyhledejte soubory v těchto umístěních:
