...
* **Microsoft. Intune. mam. SDK. DownlevelStubs. AAR**: Tento AAR obsahuje zástupné procedury pro systémové třídy Androidu, které jsou k dispozici pouze na novějších `MAMActivity`zařízeních, ale na které odkazují metody v. Novější zařízení budou tyto zástupné třídy ignorovat. Tento AAR je nutný jenom v případě, že vaše aplikace provádí reflexi `MAMActivity`u tříd odvozených z a většina aplikací je nepotřebuje zahrnout. AAR obsahuje pravidla ProGuard pro vyloučení všech jeho tříd.
...
Modul plug-in sestavení může vygenerovat sestavu HTML změn, které dělá. Chcete-li vyžádat generování této sestavy `report = true` , zadejte `intunemam` v konfiguračním bloku. Pokud je tato sestava vygenerována, bude `outputs/logs` v adresáři buildu zapsána.
...
Modul plug-in sestavení může spustit dodatečné ověření a vyhledat možné chyby ve zpracování tříd. Pokud to chcete požádat, `verify = true` zadejte v `intunemam` konfiguračním bloku. Všimněte si, že tato akce může přidat několik sekund do doby trvání úlohy modulu plug-in.
...
Chcete-li povolit podporu pro sestavení přírůstkově `incremental = true` , zadejte `intunemam` v bloku konfigurace.  Toto je experimentální funkce zaměřená na zvýšení výkonu sestavení tím, že zpracovává pouze vstupní soubory, které se změnily.  Výchozí konfigurace je `false`.
...
Některé třídy `ClipboardManager`mají většinu zabalené metody, například `ContentProviderClient` `ContentResolver`,, a `PackageManager` zatímco jiné třídy mají pouze jednu nebo dvě zabalené metody, `DownloadManager`například, `PrintManager`, `PrintHelper`, `View`, `DragEvent` `NotificationManager` a. `NotificationManagerCompat` Pokud nepoužíváte BuildPlugin, Projděte si prosím rozhraní API, které jsou vystavené ekvivalentními třídami MAM, pro přesný způsob.
...
Na rozdíl od podpůrných knihoven Androidu neposkytujeme varianty MAM knihoven AndroidX. Místo toho byste měli se sadou AndroidX nakládat jako s kteroukoli jinou externí knihovnou a měli byste ji nakonfigurovat tak, aby ji nástroj nebo modul plug-in sestavení přepsal. Pro Gradle buildy to jde udělat tak, že `androidx.*` zahrnete `includeExternalLibraries` do pole konfigurace modulu plug-in. Volání nástroje příkazového řádku musí vypsat všechny soubory jar explicitně.

### <a name="pre-androidx-architecture-components"></a>Komponenty architektury před AndroidX
Mnoho součástí architektury Androidu, včetně místností, ViewModel a WorkManager, bylo znovu zabalené pro AndroidX. Pokud vaše aplikace používá předem AndroidX varianty těchto knihoven, zajistěte, aby se přepisy použily `android.arch.*` , a `includeExternalLibraries` to zahrnutím do pole konfigurace modulu plug-in. Případně aktualizujte knihovny na jejich ekvivalenty AndroidX.
...
`username` Měl by se jednat o hlavní název uživatele (UPN)/uživatelské jméno/e-mail přidružené ke cloudové službě, která se ukládá do (*nemusí* být nutně stejná jako uživatel, který je vlastníkem ukládaného dokumentu). Pokud mapování mezi hlavním názvem uživatele služby AAD a uživatelským jménem cloudové služby neexistuje nebo není známo uživatelské jméno, použijte hodnotu null. `SaveLocation.LOCAL`není cloudová služba, takže by měla být vždy použita s parametrem `null` username.

Předchozí metoda určení, zda zásady uživatele povoluje ukládání dat do různých umístění, byla `getIsSaveToPersonalAllowed()` v rámci stejné třídy **AppPolicy** . Ta je teď **zastaralá** a neměla by se používat. Vyvolání ekvivalentní k funkci `getIsSaveToPersonalAllowed()` vidíte níže:
...
Pokud je `BLOCKED`omezení, aplikace nesmí zobrazit žádná oznámení pro uživatele přidruženého k této zásadě. V `BLOCK_ORG_DATA`případě, že aplikace musí zobrazit upravené oznámení, které neobsahuje data organizace. V `UNRESTRICTED`případě, že jsou povolena všechna oznámení.

Pokud `getNotificationRestriction` není vyvolána, sada mam SDK se bude snažit vyhradit automatické omezení oznámení pro aplikace s jedinou identitou. Pokud je zapnuté automatické blokování `BLOCK_ORG_DATA` a je nastavené, nebude se vám zobrazovat oznámení. V případě podrobnějšího řízení ověřte hodnotu `getNotificationRestriction` a patřičně upravte oznámení aplikace.
...
2. Metoda `acquireToken()` by měla získat přístupový token pro požadované ID prostředku daného uživatele. Pokud ho získat nedokáže, měla by vrátit hodnotu null.
    ...
    > [!NOTE]
    > Ujistěte se, že vaše aplikace využívá `resourceId` parametry `aadId` a předané `acquireToken()` do tak, aby byl získán správný token.
...
> [!NOTE]
> Nenastavujte položku `com.microsoft.intune.mam.aad.Authority` meta-data v souboru souboru AndroidManifest. XML.
...
Stav zaregistrovaného uživatelského účtu se může po přijetí oznámení o registraci změnit, ale ve všech případech se nezmění (například pokud `AUTHORIZATION_NEEDED` je oznámení obdrženo po více informativních výsledcích, jako je například `WRONG_USER`, bude se uchovávat další informativní výsledek jako stav účtu).  Po úspěšné registraci účtu zůstane stav shodný s `ENROLLMENT_SUCCEEDED` tím, že se účet nebude registrovat nebo není smazán.
...
Knihovna ADAL má nový kód chyby, který informuje aplikaci o tom, že selhání získání tokenu bylo způsobeno tím, že není kompatibilní se správou aplikací.  Pokud aplikace obdrží tento kód chyby, musí volat sadu SDK, aby se pokusila opravit dodržování předpisů tím, že zaregistruje aplikaci a použije zásady. `onError()` Metodou ADAL `AuthenticationCallback`bude přijata výjimka, která bude obsahovat kód `ADALError.AUTH_FAILED_INTUNE_POLICY_REQUIRED`chyby.  V takovém případě může být výjimka převedena na `IntuneAppProtectionPolicyRequiredException`, ze kterého mohou být extrahovány další parametry pro použití v Oprava dodržování předpisů (viz Ukázka kódu níže). Po úspěšné opravě se aplikace může znovu pokusit o získání tokenu prostřednictvím ADAL.
...
> [!NOTE]
> Příjemce oznámení musí být před voláním `remediateCompliance()` zaregistrován, aby nedocházelo ke konfliktu časování, který by mohl způsobit vynechání oznámení.

### <a name="implementation-notes"></a>Poznámky k implementaci
> [!NOTE]
> **Důležitá změna!**  <br>
> Metoda aplikace by měla předat *hodnotu false* pro nový `forceRefresh` příznak `acquireTokenSilentSync()` `MAMServiceAuthenticationCallback.acquireToken()`
...
Příručka pro zálohování dat určuje obecný algoritmus pro obnovení dat vaší aplikace a poskytuje ukázku kódu v části [rozšíření třídy backupagent](https://developer.android.com/guide/topics/data/keyvaluebackup.html#BackupAgent) . Pokud má obnova více identit proběhnout úspěšně, je potřeba dodržet obecnou strukturu danou tímto vzorovým kódem a věnovat zvláštní pozornost těmto bodům:

1. K přechodu mezi entitami zálohování je nutné použít `while(data.readNextHeader())` smyčku. V předchozím kódu `data` je název místní proměnné pro **MAMBackupDataInput** , který se předá do vaší aplikace při obnovení.

2. Je nutné zavolat `data.skipEntityData()` , `data.getKey()` Pokud neodpovídá klíči, který jste napsali `onBackup`v. Pokud tento krok neprovedete, záloha nemusí proběhnout úspěšně. V předchozím kódu `data` je název místní proměnné pro **MAMBackupDataInput** , který se předá do vaší aplikace při obnovení.

3. Vyhněte se vrácení při využívání entit zálohování `while(data.readNextHeader())` v konstrukci, protože entity, které automaticky píšete, budou ztraceny. V předchozím kódu `data` je název místní proměnné pro **MAMBackupDataInput** , který se předá do vaší aplikace při obnovení.
...
Identita nastavená na úrovni vlákna nahrazuje identitu nastavenou na úrovni objektu `Context`, která nahrazuje identitu nastavenou na úrovni procesu. Identita nastavená pro `Context` se používá jenom v příslušných přidružených scénářích. Operace v/v souborů nemají například přidruženy `Context`. Nejčastěji budou aplikace nastavovat `Context` identitu na. `Activity` Aplikace *nesmí* zobrazit data pro spravovanou identitu, pokud není `Activity` identita nastavená na stejnou identitu. Obecně je identita na úrovni procesu užitečná pouze tehdy, když pracuje aplikace ve všech vláknech vždy jen s jediným uživatelem. Řada aplikací tuto možnost nemusí potřebovat využít.

Pokud vaše aplikace používá `Application` kontext k získání systémových služeb, ujistěte se, že byla nastavena identita vlákna nebo procesu nebo zda jste nastavili IDENTITU uživatelského rozhraní v `Application` kontextu vaší aplikace.

Chcete-li zpracovat zvláštní případy při aktualizaci identity uživatelského `setUIPolicyIdentity` rozhraní `switchMAMIdentity`pomocí nebo, lze obě metody předat sadu `IdentitySwitchOption` hodnot.
...
>[!NOTE]
> `CLIPBOARD_SERVICE` Vzhledem k tomu, že se používá pro operace uživatelského rozhraní, sada SDK používá identitu uživatelského rozhraní aktivity `ClipboardManager` popředí pro operace.
...
| `NOT_ALLOWED`  | Změna identity není povolená. K tomu dojde, pokud je proveden pokus o nastavení identity uživatelského`Context`rozhraní (), pokud je v aktuálním vlákně nastavena jiná identita. |
...
V případě nastavení `Context` identity prostřednictvím `setUIPolicyIdentity`je výsledek hlášen asynchronně. Pokud `Context` je to `Activity`, sada SDK neví, zda byla změna identity úspěšná, až po podmíněném spuštění, které může vyžadovat, aby uživatel zadal PIN nebo firemní přihlašovací údaje. Aplikace může implementovat a `MAMSetUIIdentityCallback` získat tento výsledek nebo může pro objekt zpětného volání předat hodnotu null. Všimněte si, že pokud je učiněno `setUIPolicyIdentity` volání, zatímco výsledek z předchozího volání `setUIPolicyIdentity` *na stejný kontext* nebyl dosud doručen, nové zpětné volání nahradí starý a původní zpětné volání nikdy neobdrží výsledek.
...
* `reportIdentitySwitchResult(FAILURE)`je `RESUME_CANCELLED`-li důvod.
...
* `reportIdentitySwitchResult(SUCCESS)` ve všech ostatních případech.
  ...
  * Pokud služba běží na hlavním vlákně, `reportIdentitySwitchResult` **musí** se  volat synchronně, jinak přestane vlákno uživatelského rozhraní reagovat.
  ...
  * Pokud je **`Activity`** v případě, že je požadován přepínač na prázdnou identitu s odůvodněním `RESUME_CANCELLED`, aplikace musí upravit obnovenou aktivitu a zobrazit tak data konzistentní s tímto přepínačem identity.  Pokud to není možné, aplikace by měla přepnutí odmítnout a uživatel se znovu vyzve k dodržování zásad pro obnovení identity (například zobrazením obrazovky pro zadání kódu PIN).
...
Chcete- `MAMAsyncTask`li použít, jednoduše dědit z něj `AsyncTask` namísto a nahraďte `doInBackground` a `onPreExecute` v `doInBackgroundMAM` a `onPreExecuteMAM` v. Konstruktor `MAMAsyncTask` převezme kontext aktivity. Příklad:
...
Každý soubor má identitu, která je s ním spojená v době vytvoření, podle identity vláken a procesů. Tato identita se používá pro šifrování souborů i selektivní vymazání. Šifrovat se budou jen soubory, jejichž identita je spravovaná a má zásady vyžadující šifrování. Výchozí selektivní vymazání funkcí sadou SDK vymaže jen soubory spojené s identitou, u které se požaduje vymazání. Aplikace může zadat dotaz na identitu souboru nebo ji `MAMFileProtectionManager` změnit pomocí třídy.
...
Pokud aplikace `ParcelFileDescriptor` poskytuje podniková data kromě přes a `ContentProvider`, musí aplikace zavolat metodu `isProvideContentAllowed(String)` v `MAMContentProvider`, která předá hlavní název uživatele (UPN) identity vlastníka. Pokud tato funkce vrací hodnotu False, *nesmí* se obsah vrátit zpět volajícímu. Popisovače souborů, které se vrátí přes poskytovatele obsahu, se automaticky zpracují podle identity souboru.
...
> [!NOTE] 
> Nastavení konfigurace pro doručení prostřednictvím MAM – nemůžeme delievered v `offline` (Pokud není nainstalovaná portál společnosti).  V tomto případě se v takovém případě doručí `MAMUserNotification` v prázdné identitě jenom Android Enterprise AppRestrictions.
...
Chcete-li určit motiv aplikace pomocí sady MAM SDK, je nutné do `onCreate` metody aplikace přidat následující řádek kódu:
...
* **Použití překladačů obsahu**: Zásady Intune o přenosu nebo příjmu můžou blokovat nebo částečně blokovat překladače obsahu použité při přístupu k poskytovateli obsahu v jiné aplikaci. To způsobí, `ContentResolver` že metody vrátí hodnotu null nebo vyvolají chybovou hodnotu (například `openOutputStream` vyvolá výjimku `FileNotFoundException` , pokud se zablokuje). Aplikace může určit, jestli selhání při zápisu dat pomocí překladače obsahu způsobila zásada (nebo by způsobila zásada), a to voláním:
