...
Nezapomeňte pro aplikace Office [Povolit automatické aktualizace aplikací pro Microsoft 365](/deployoffice/configure-update-settings-for-office-365-proplus) . Můžete to provést pomocí zásad skupiny nebo šablony Intune Office 2016 ADMX:
...
2. Nezapomeňte [tuto zásadu přiřadit](device-profile-assign.md) k zařízením s Windows 10. Pokud chcete zásady testovat dřív, můžete taky tuto zásadu synchronizovat:
    ...
    - [Ruční synchronizace zásad na zařízení](../user-help/sync-your-device-manually-windows.md#sync-from-settings-app)
...
2. Přejít na cestu k zásadám Intune: `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PolicyManager\Providers\<Provider ID>\default\Device\office16~Policy~L_MicrosoftOfficemachine~L_Updates` .
    ...
    > [!TIP]
    > [Spravujte Microsoft 365 aplikace pomocí Configuration Manager](/configmgr/sum/deploy-use/manage-office-365-proplus-updates#bkmk_channel) vypíše hodnoty a jejich význam. Hodnoty registru jsou založené na vybraném distribučním kanálu:
    >
...
1. Potvrďte, že verze Office podporuje kanál pro aktualizaci, který si zvolíte. [Historie aktualizací pro aplikace Microsoft 365](/officeupdates/update-history-office365-proplus-by-date) obsahuje čísla sestavení, která podporují různé kanály aktualizace.
