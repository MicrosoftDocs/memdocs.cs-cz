...
Když použijete aktuální větev Configuration Manager, můžete nainstalovat konzolovou aktualizaci pro verzi 2002, která aktualizuje vaši hierarchii z předchozí verze. <!-- baseline only statement:-->Verze 2002 je také k dispozici jako [Základní médium](updates.md#bkmk_Baselines), takže k instalaci první lokality nové hierarchie můžete použít instalační média.

Chcete-li získat aktualizaci verze 2002, je nutné použít spojovací bod služby v lokalitě nejvyšší úrovně ve vaší hierarchii. Tato role systému lokality může být v režimu online nebo offline. Pokud chcete stáhnout aktualizaci, když je spojovací bod služby offline, [použijte nástroj pro připojení služby](use-the-service-connection-tool.md).<!-- SCCMDocs#1946 -->
...
- Je-li aktualizace uvedena jako **dostupná**, je aktualizace připravena k instalaci. Před instalací verze 2002 si přečtěte následující informace [o instalaci aktualizace 2002](#about-installing-update-2002) a [Kontrolní seznam](#checklist) pro konfigurace, které se mají provést před zahájením aktualizace.
...
## <a name="about-installing-update-2002"></a>O instalaci aktualizace 2002

### <a name="sites"></a>Lokality

Nainstalujte aktualizaci 2002 v lokalitě nejvyšší úrovně ve vaší hierarchii. Spusťte instalaci z lokality centrální správy (CAS) nebo ze samostatné primární lokality. Po instalaci aktualizace v lokalitě nejvyšší úrovně mají podřízené lokality následující chování aktualizace:
...
Aby bylo možné spustit instalaci aktualizace 2002, musí každý server lokality v hierarchii používat stejnou verzi Configuration Manager. K aktualizaci na 2002 je nutné použít verzi 1810 nebo novější.
...
<!--SCCMdocs#587-->
Pokud rozšíříte Configuration Manager s jinými produkty od partnerů Microsoftu nebo Microsoftu, ujistěte se, že tyto produkty podporují verzi 2002. Tyto informace vám poskytne dodavatel produktu. Příklad najdete v [poznámkách k verzi](../../../mdt/release-notes.md)Microsoft Deployment Toolkit.
...
Chcete-li spustit kontrolu požadovaných součástí z konzoly, otevřete ovládací prostor pro **správu** a vyberte možnost **aktualizace a údržba**. Vyberte balíček aktualizace **Configuration Manager 2002** a na pásu karet vyberte **Spustit kontrolu požadovaných součástí** .
...
Zajistěte, aby byl každý server lokality a role systému lokality aktualizované na verzi 2002. V konzole nástroje přidejte sloupec **verze** do uzlů **lokality** a **distribučních bodů** v pracovním prostoru **Správa** . V případě potřeby se role systému lokality automaticky přeinstaluje, aby se aktualizovala na novou verzi.
...
Pokud používáte jakákoli rozšíření Configuration Manager, aktualizujte je na nejnovější verzi, aby podporovala Configuration Manager verze 2002.
